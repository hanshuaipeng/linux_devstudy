...
static int __init gpioled_init(void)
{
	...
	/* 获取设备树中的属性数据 */
	 /* 1、获取设备节点： led */
	gpioled.nd = of_find_node_by_path("/gpio_beep");
	if(gpioled.nd == NULL)
	{
		printk("can't find gpio_beep\r\n");
		return -EINVAL;
	}
	else
	{
		printk("gpio_beep node has been found! \r\n");
	}
	/* 2、 获取设备树中的 gpio 属性，得到 BEEP 所使用的 BEEP 编号 */

	gpioled.beep_gpio = of_get_named_gpio(gpioled.nd, "beep-gpios", 0);
